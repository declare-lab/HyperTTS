...
def train(rank, args, configs, batch_size, num_gpus):
    ...
    # Prepare model
    model, optimizer, scheduled_optim = get_model(args, configs, device, train=True)
    if num_gpus > 1:
    ...
    if rank == 0:
        for n,p in model.named_parameters():
            if ("hypernet" not in n): #and ("mel_linear" not in n):
                p.requires_grad = False
        print("Number of CompTransTTS Parameters: {}\n".format(get_param_num(model)))
        for n,p in model.named_parameters():
            if p.requires_grad:
                print(n)
        if model_config["hyperx"]["required"]:
            print("Number of Hyperx Parameters: {}\n".format(
                get_param_num(model.decoder.hypernet)+get_param_num(model.decoder.hypernet_source_controller)))
    ...
    while train:
        ...
        for batchs in loader:
            ...
            for batch in batchs:
                ...
                ##changed
                #'''
                lr = np.array(scheduled_optim.get_last_lr())
                        ...
                        torch.save(
                            {
                                ...
                                "optimizer": optimizer.state_dict(),#._optimizer.state_dict(),
                            },
                            os.path.join(
