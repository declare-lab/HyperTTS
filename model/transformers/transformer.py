...
class TextEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config["hyperx"]["required"]:
            ...
            self.hypernet_source_controller = SourceController(config, config[config["block_type"]]["encoder_layer"])
        #'''

    def forward(self, src_seq, mask, speaker_embeds, return_attns=False):
        ...
        for i, enc_layer in enumerate(self.layer_stack):
            ...
            #'''
            if self.config["hyperx"]["required"] and not self.config["hyperx"]["after_layer_output"]:
                if self.config["hyperx"]["condition_to_layer_id"]:
                    source_embedding = self.hypernet_source_controller(speaker_embeds, f'{i}')
                    enc_layer.pos_ffn.set_hypernet(self.hypernet, source_embedding)
                else:
                    source_embedding = self.hypernet_source_controller(speaker_embeds)
                    enc_layer.pos_ffn.set_hypernet(self.hypernet[i], source_embedding)
            #'''
...
class Decoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config["hyperx"]["required"]:
            ...
            self.hypernet_source_controller = SourceController(config, config[config["block_type"]]["decoder_layer"])
        #'''

    def forward(self, enc_seq, mask, speaker_embeds, return_attns=False):
        ...
        for i, dec_layer in enumerate(self.layer_stack):
            ...
            if self.config["hyperx"]["required"] and not self.config["hyperx"]["after_layer_output"]:
                if self.config["hyperx"]["condition_to_layer_id"]:
                    source_embedding = self.hypernet_source_controller(speaker_embeds, f'{i}')
                    dec_layer.pos_ffn.set_hypernet(self.hypernet, source_embedding)
                    # breakpoint()
                else:
                    source_embedding = self.hypernet_source_controller(speaker_embeds)
                    dec_layer.pos_ffn.set_hypernet(self.hypernet[i], source_embedding)
            #'''
...
class PositionwiseFeedForward(nn.Module):
    ...
    def forward(self, x):
        ...
        #'''  M2 adapters via hypernetwork(s)
        if self.hypernet:
            output = self.hypernet(self.source_embedding, output)
        #'''
